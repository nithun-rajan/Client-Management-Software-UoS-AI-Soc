...
class LLMService:
    ...
    async def estimate_monthly_rent(
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Error building prompt: {e!s}",
                "estimated_rent": None,
            }
...
price properties competitively while maximizing returns.""",
            ...
            return {"success": False, "error": str(e), "estimated_rent": None}
    ...
    def _build_rent_estimation_prompt(self, property_data: dict[str, Any]) -> str:
        ...
        self._safe_dict(address.get("royal_mail_format"))
        self._safe_dict(address.get("simplified_format"))
        ...
        prop_type = property_data.get(
            "property_type", property_type.get("value", "Unknown")
        )
        bedrooms = property_data.get(
            "bedrooms", characteristics.get("num_bedrooms", "Not specified")
        )
        bathrooms = property_data.get(
            "bathrooms", characteristics.get("num_bathrooms", "Not specified")
        )
        floor_area = property_data.get(
            "floor_area_sqm", characteristics.get("total_floor_area", "Not specified")
        )

        # Build prompt
        return f"""Analyze this UK property and estimate the optimal monthly rent for letting.
