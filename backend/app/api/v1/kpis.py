...
def get_lettings_performance(db: Session = Depends(get_db)):
    ...
    # Calculate the average difference in days between the two timestamps
    # Using SQLite-compatible julianday() function
    # For PostgreSQL, you could use: func.avg(func.extract('epoch', t2.created_at - t1.created_at)) / (60 * 60 * 24)
    avg_progression_days_query = db.query(
        func.avg(func.julianday(t2.created_at) - func.julianday(t1.created_at))
    ).join(
        t2, and_(
    ...
    avg_progression_days = avg_progression_days_query.scalar() or 0

    # --- 2. Average Referencing Time ---
    # (Time from "referencing" to "referenced")
    avg_referencing_days_query = db.query(
        func.avg(func.julianday(t2.created_at) - func.julianday(t1.created_at))
    ).join(
        t2, and_(
            t1.entity_id == t2.entity_id,
    ...
    avg_referencing_days = avg_referencing_days_query.scalar() or 0
