...
class PropertyMatcher:
    ...
    def calculate_match_score(property: Property, applicant: Applicant) -> float:
        ...
        max_score += 15
        if applicant.desired_property_type and (
            applicant.desired_property_type.lower() in property.property_type.lower()
        ):
            score += 15
    ...
    def generate_personalized_message(
        ...
        if score >= 90:
            message_parts.append(
                "I've found an amazing property that's perfect for you! "
            )
        elif score >= 75:
        ...
        if applicant.has_pets and applicant.pet_details:
            ...
            if property.features and "garden" in property.features.lower():
                message_parts.append(
                    f"Great news - it has a garden that {applicant.pet_details} would love! "
                )
            elif property.features and "pets_allowed" in property.features.lower():
                message_parts.append(
                    f"Good news - pets are welcome here for {applicant.pet_details}! "
                )
        ...
        if applicant.special_requirements:
            ...
            if "school" in reqs and property.features:
                ...
                if "school" in features_lower:
                    message_parts.append(
                        "Perfect for families - it's near excellent schools! "
                    )
