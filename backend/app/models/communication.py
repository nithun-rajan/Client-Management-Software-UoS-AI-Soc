...
class Communication(Base):
    ...
    )  # email, call, sms, note, task, meeting, viewing
    ...
    )  # inbound, outbound (null for notes/tasks)
    ...
    applicant_id = Column(
        Integer, ForeignKey("applicants.id"), nullable=True, index=True
    )
    ...
    def __repr__(self):
        entity = (
            ...
            if self.property_id
            else f"Landlord#{self.landlord_id}"
            if self.landlord_id
            else f"Applicant#{self.applicant_id}"
