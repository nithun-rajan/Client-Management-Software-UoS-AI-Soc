...
export default function Valuations() {
  ...
  const handleGenerateValuationPack = async (property: Property) => {
    ...
    try {
      ...
      // Use NEW AI-powered endpoints based on property type
      const isSalesProperty = property.sales_status !== null && property.sales_status !== undefined;
      const endpoint = isSalesProperty 
        ? "/api/v1/land-registry/ai-sales-valuation"
        : "/api/v1/land-registry/ai-lettings-valuation";
      ...
      toast.success("AI-powered valuation generated successfully");
    } catch (error: any) {
      toast.error(
        error?.response?.data?.detail || "Failed to generate AI valuation"
      );
  ...
  const handleViewValuationPack = async (property: Property) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.detail || "Failed to load AI valuation"
      );
      setValuationPackOpen(false); // Close dialog on error
    } finally {
      setGeneratingPack(false);
              ...
              <CardContent className="space-y-4">
                ...
                {valuation.date && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground pt-2 border-t">
                    ...
                    <span>
                      {format(new Date(valuation.date), "dd/MM/yyyy")}
                    </span>
                  </div>
                )}
              </CardContent>
